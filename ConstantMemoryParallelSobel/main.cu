...
__constant__ int constMaskH[SOBEL_OP_DIM * SOBEL_OP_DIM];
__constant__ int constMaskV[SOBEL_OP_DIM * SOBEL_OP_DIM];
...
__global__ void gradientConvolutionV(uint8_t* inputImgData, uint8_t* gradientImgData, int w, int h) {
...
__global__ void gradientConvolutionH(uint8_t* inputImgData, uint8_t* gradientImgData, int w, int h) {
...
int main(int argc, char** argv) {
	...
	// copy mask on constant memory
	HANDLE_ERROR(cudaMemcpyToSymbol(constMaskH, sobel_h, SOBEL_OP_DIM * SOBEL_OP_DIM * sizeof(int)));
	std::cout << " DONE" << std::endl;
	...
	// copy mask on device
	HANDLE_ERROR(cudaMemcpyToSymbol(constMaskV, sobel_v, SOBEL_OP_DIM* SOBEL_OP_DIM * sizeof(int)));
